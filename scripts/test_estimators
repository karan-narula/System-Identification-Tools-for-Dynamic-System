...
def test_fit_data_rover(param_dict, num_mc=100, back_rotate=False, **kwargs):
    ...
    for i in range(num_mc):
        ...
        np.random.seed(i)
        dynamic_obj = create_dyn_obj(RoverPartialDynEst,
                                     param_dict,
                                     simulate_gt=True,
                                     real_output=False,
                                     **kwargs)
        ...
        vy = yawrate * (
            dynamic_obj.param_dict['c8'] + dynamic_obj.param_dict['c9'] *
            dynamic_obj.gt_states[dynamic_obj.state_dict['vx'], :]**2)

        if back_rotate and dynamic_obj.state_dict[
        ...
        dynamic_obj.sample_nlds(dynamic_obj.initial_cond, dynamic_obj.U,
                                dynamic_obj.T)
    ...
    plt.plot(i_gt_states[dynamic_obj.state_dict['x'], :],
             i_gt_states[dynamic_obj.state_dict['y'], :],
             label='gt')
    plt.plot(i_approx_states[dynamic_obj.state_dict['x'], :],
             i_approx_states[dynamic_obj.state_dict['y'], :],
             label='nls_fit')
    plt.grid(True, "both")
    plt.legend()
    plt.xlabel('X (m)')
    ...
    plt.ylabel('MSE')
    plt.title(
        'MSE at different time instant from {} mc simulations'.format(num_mc))
    plt.show()
...
def test_pbgf(dyn_class,
    ...
    # get data
    dynamic_obj = create_dyn_obj(dyn_class,
                                 param_dict,
                                 simulate_gt=True,
                                 real_output=False,
                                 re_initialise=len(ode_vars) == 0,
                                 **kwargs)

    # optionally, use ode solver for the dynamic system and compare the results
    if len(ode_vars):
        if 'T' in ode_vars and sample_linear_flag:
            U = sample_linear(ode_vars['T'], cruise_time, decrease_flag,
                              *max_inputs_list)
            ode_vars['U'] = U
    ...
    assert operation in ['filter',
                         'smoother'], "Invalid estimation operation requested"
    if operation == 'filter':
        ...
        est_states = create_filtered_estimates(
            ...
            tensor_device=tensor_device)[0]
    else:
        lag_interval = kwargs.get('lag_interval', 5)
        est_states = create_smoothed_estimates(dynamic_obj,
                                               order=2,
                                               lag_interval=lag_interval)[0]
    ...
    plot_stuff(dynamic_obj,
               est_states,
               angle_states=kwargs.get('angle_states', []),
               encapsulated_gt=True,
               num_rows=[2, 2])
...
if __name__ == '__main__':
    ...
    input_vars = {
        'sample_linear_flag': True,
        'max_inputs_list': max_inputs_list
    }

    test_pbgf(RoverPartialDynEst,
              param_dict,
    ...
    test_pbgf(FrontDriveFrontSteerEst,
              param_dict,
              input_vars=input_vars,
              **configuration)

    # test pbgf for FrontDriveFrontSteer with sudden change in fr
    param_dict = ODict([('fx', 15), ('cf', 0.75), ('cr', [0.75, 1.0]),
                        ('lf', 0.8), ('lr', 1.75), ('m', 1000.0),
                        ('iz', 100.0), ('rc', 0.0), ('fr', [1.0, 2.0]),
                        ('g', 9.8)])
    configuration = {}
    configuration = {
        'seed': 0,
    ...
    param_dict = ODict([('sigma_0', 40.0), ('sigma_1', 4.9487),
                        ('sigma_2', 0.0018), ('sigma_w', 0.0), ('L', 0.25),
                        ('mu_c', 0.5), ('mu_s', 0.9), ('vs', 12.5),
                        ('r', 0.25), ('m', 5.0), ('J', 0.2344), ('Fn', 14.0),
                        ('k', 1.1), ('theta', [1.0, 2.0])])
    configuration = {}
    configuration = {
        'seed': 0,
    ...
    ode_vars = {
        'T': np.arange(0.0, timing_vars['tf'], 0.1),
        'plot_result': True,
        'plot_euler_result': True,
        'num_rows': 1
    }

    test_pbgf(OneWheelFrictionEst,
              param_dict,
    ...
    param_dict = ODict([('sigma_0', 200.0), ('sigma_1', 0.4948),
                        ('sigma_2', 0.0018), ('sigma_w', 0.0), ('L', 0.2),
                        ('mu_c', 0.7), ('mu_s', 1.9), ('vs', 3.5), ('r', 0.25),
                        ('m', 350.0), ('J', 0.2344), ('Fn', 1000.0),
                        ('k', 1.1), ('theta', [1.0, 2.0])])
    configuration = {}
    configuration = {
        'seed': 0,
